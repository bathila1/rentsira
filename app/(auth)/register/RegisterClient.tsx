...
import {
  formatAndValidateSLNumber,
  sanitizeEmail,
  sanitizeText,
} from "@/utils/sanitize";

// ← removed: import type { Metadata }
...
export default function RegisterClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const sanitizedEmailForm = sanitizeEmail(form.email);
    if (!sanitizedEmailForm) {
    ...
    if (error) {
      ...
      const { error: signInError } = await supabase.auth.signInWithPassword({
        email: sanitizedEmailForm as string,
        password: form.password,
      });
